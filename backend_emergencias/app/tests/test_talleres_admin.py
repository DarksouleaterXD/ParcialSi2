"""Tests de la API /api/admin/talleres."""
...
def test_admin_crear_listar_y_desactivar(client):
    token = _login(client)
    r = client.post("/api/admin/talleres", json=_taller_body(nombre="Taller Alpha"), headers=_hdr(token))
    assert r.status_code == 201
    row = r.json()
    assert row["nombre"] == "Taller Alpha"
